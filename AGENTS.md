...
- 热力图使用周日在上的 GitHub 式 7 行布局和小圆角方格，在卡片左边缘与模型区域的竖向分隔线之间水平居中，两侧留白相等；零用量使用主题自适应的中性灰，有用量按六档高区分度蓝色色阶显示，低档接近嵌套背景、中档逐步增强、最高两档同时向主题主文字色拉开明暗；不显示月份、星期或图例，今天使用蓝色细描边。
