...
- 右侧环形图不显示独立小标题，并与热力图顶部对齐；它按调用次数显示前 5 个模型，其余合并为“其他模型”。中心显示模型统计总调用数，图例和交互提示显示模型、调用次数及占比；窄屏时图表自动排列到热力图下方。
