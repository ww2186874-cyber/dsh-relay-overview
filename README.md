...
- 热力图采用周日在上的 GitHub 式 7 行布局，并在卡片左边缘与中间分隔线之间水平居中；零用量显示主题自适应的中性灰，有用量按六档高区分度蓝色色阶显示。悬停、聚焦或触屏点击某天可查看日期、扣费、请求数和 Token。
