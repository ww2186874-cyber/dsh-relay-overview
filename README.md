...
- 订阅接口同时提供到期时间和额度窗口起点时，鼠标悬停进度条只显示 `剩余N天（YYYY/MM/DD HH:mm） DdHh后重置`。
...
- Subscription: uses the configured daily/weekly/monthly constraint with the smallest remaining amount. When the upstream supplies expiry and window-start timestamps, hovering the expanded progress track shows only the subscription days/date and quota-reset countdown.
