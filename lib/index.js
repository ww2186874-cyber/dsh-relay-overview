...
function shanghaiDateKey(date) {
  ...
  if (!Number.isFinite(milliseconds)) throw new Error('dsh-relay-balance now() must return a valid Date')
...
export function resolveRelayConfig(providerSettings, config = DEFAULT_CONFIG, relaySettings = {}) {
  ...
  if (record(provider) === null) throw new PublicBalanceError('provider-not-configured', '请在中转概览设置中填写中转 URL 和 API Key', 503)
  const baseURL = typeof provider.baseURL === 'string' ? provider.baseURL.trim() : ''
  const credentialRef = typeof provider.apiKeyEnv === 'string' ? normalizeCredentialRef(provider.apiKeyEnv) : ''
  if (baseURL === '' || credentialRef === '') throw new PublicBalanceError('provider-not-configured', '请在中转概览设置中填写中转 URL 和 API Key', 503)
  return {
    displayName: normalized.displayName,
