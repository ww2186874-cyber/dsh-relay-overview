...
const STATUS_ROUTE_PATH = '/relay-overview/status'
const HISTORY_ROUTE_PATH = '/relay-overview/history'
const TEST_ROUTE_PATH = '/relay-overview/test'
const SAVE_ROUTE_PATH = '/relay-overview/save'
const SETTINGS_NAMESPACE = 'dsh-relay-overview'
const MANAGED_CREDENTIAL_PREFIX = 'DSH_RELAY_OVERVIEW_'
const HISTORY_DAYS = 30
const HISTORY_TIME_ZONE = 'Asia/Shanghai'
const MODEL_USAGE_LIMIT = 5
const MAX_MODEL_STATS_ROWS = 500
...
export const name = 'dsh-relay-overview'
...
function requireMethod(owner, serviceName, methodName) {
  if (typeof owner?.[methodName] !== 'function') throw new Error(`dsh-relay-overview requires ${serviceName}.${methodName}()`)
}
...
function normalizeUsagePath(value) {
  ...
  if (usagePath !== 'auto' && !/^\/[A-Za-z0-9._~!$&'()*+,;=:@%/-]*$/.test(usagePath)) {
    throw new Error("dsh-relay-overview usagePath must be 'auto' or an absolute URL path without query or fragment")
  }
  return usagePath
}
...
function normalizeCredentialRef(value) {
  ...
  if (credentialRef !== '' && !/^[A-Za-z_][A-Za-z0-9_]*$/.test(credentialRef)) {
    throw new Error('dsh-relay-overview credentialRef must be a POSIX-style environment variable name')
  }
...
export function normalizePluginConfig(value = {}) {
  ...
  if (displayName === '' || displayName.length > 64) throw new Error('dsh-relay-overview config.displayName must contain 1-64 characters')
  if (baseURL !== '') buildUsageUrls(baseURL, usagePath)
  if ((baseURL === '') !== (credentialRef === '')) throw new Error('dsh-relay-overview config.baseURL and config.credentialRef must be set together')
  return { displayName, baseURL, credentialRef, usagePath, allowRemote }
}
...
function shanghaiDateKey(date) {
  ...
  if (!Number.isFinite(milliseconds)) throw new Error('dsh-relay-overview now() must return a valid Date')
  return new Date(milliseconds + SHANGHAI_OFFSET_MS).toISOString().slice(0, 10)
}
...
function finiteSafeCount(value, field) {
  const count = finiteNonNegative(value, field)
  if (!Number.isSafeInteger(count)) throw publicInvalidData(`中转站返回的 ${field} 无效`)
  return count
}

function finiteSafeCost(value, field) {
  const cost = finiteNonNegative(value, field)
  if (cost > Number.MAX_SAFE_INTEGER) throw publicInvalidData(`中转站返回的 ${field} 无效`)
  return cost
}
...
function modelName(value) {
  ...
  if (name === '' || name.length > MAX_MODEL_NAME_LENGTH || /[\u0000-\u001f\u007f-\u009f]/.test(name)) return null
  return name
}

export function normalizeSub2ApiModelUsage(value) {
  if (value === undefined || value === null) return null
  if (!Array.isArray(value) || value.length > MAX_MODEL_STATS_ROWS) return null
  const totals = new Map()
  for (const item of value) {
    const entry = record(item)
    const model = modelName(entry?.model)
    const requests = entry?.requests
    if (model === null || !Number.isSafeInteger(requests) || requests < 0) return null
    const total = (totals.get(model) ?? 0) + requests
    if (!Number.isSafeInteger(total)) return null
    totals.set(model, total)
  }
  const ranked = [...totals].map(([model, requests]) => ({ model, requests }))
    .filter((entry) => entry.requests > 0)
    .sort((a, b) => b.requests - a.requests || (a.model < b.model ? -1 : a.model > b.model ? 1 : 0))
  let totalRequests = 0
  for (const entry of ranked) {
    totalRequests += entry.requests
    if (!Number.isSafeInteger(totalRequests)) return null
  }
  const models = ranked.slice(0, MODEL_USAGE_LIMIT)
  const topRequests = models.reduce((total, entry) => total + entry.requests, 0)
  return { totalRequests, models, otherRequests: totalRequests - topRequests }
}

export function normalizeSub2ApiHistory(payload, options = {}) {
  if (record(payload) === null || payload.isValid !== true) throw publicInvalidData()
  if (!Array.isArray(payload.daily_usage)) throw publicInvalidData('中转站未提供每日使用记录')
  if (payload.daily_usage.length > 366) throw publicInvalidData('中转站返回的每日记录过多')
  const now = options.now ?? new Date()
  const through = shanghaiDateKey(now)
  const dateKeys = Array.from({ length: HISTORY_DAYS }, (_, index) => dateKeyDaysBefore(through, HISTORY_DAYS - 1 - index))
  const allowedDates = new Set(dateKeys)
  const byDate = new Map()
  for (const value of payload.daily_usage) {
    const entry = record(value)
    if (entry === null) throw publicInvalidData('中转站返回的每日记录无效')
    const date = calendarDateKey(entry.date)
    if (date === null) throw publicInvalidData('中转站返回的 daily_usage.date 无效')
    if (!allowedDates.has(date)) continue
    if (byDate.has(date)) throw publicInvalidData(`中转站返回了重复的每日记录：${date}`)
    byDate.set(date, {
      date,
      actualCost: finiteSafeCost(entry.actual_cost, 'daily_usage.actual_cost'),
      requests: finiteSafeCount(entry.requests, 'daily_usage.requests'),
      totalTokens: finiteSafeCount(entry.total_tokens, 'daily_usage.total_tokens'),
    })
  }
  const days = dateKeys.map((date) => byDate.get(date) ?? { date, actualCost: 0, requests: 0, totalTokens: 0 })
  ...
  if (!Number.isFinite(summary.actualCost) || !Number.isSafeInteger(summary.requests) || !Number.isSafeInteger(summary.totalTokens)) {
    throw publicInvalidData('中转站返回的 30 天用量汇总无效')
  }
  const fetchedAt = typeof options.fetchedAt === 'string' ? options.fetchedAt : now.toISOString()
  return {
    adapter: 'sub2api',
    unit: unitOf(payload),
    ...
    days,
    summary,
    modelUsage: normalizeSub2ApiModelUsage(payload.model_stats),
    fetchedAt,
  }
}
...
export function buildUsageUrls(baseURL, usagePath = 'auto') {
  ...
  try {
    ...
    throw new PublicBalanceError('relay-not-configured', 'Relay baseURL 无效', 503)
  }
  if (parsed.protocol !== 'https:') throw new PublicBalanceError('relay-not-configured', 'Relay baseURL 必须使用 HTTPS', 503)
  if (parsed.username !== '' || parsed.password !== '' || parsed.search !== '' || parsed.hash !== '') {
    throw new PublicBalanceError('relay-not-configured', 'Relay baseURL 不得包含凭据、查询参数或片段', 503)
  }
...
export function resolveRelayConfig(config = DEFAULT_CONFIG, relaySettings = {}) {
  ...
  throw new PublicBalanceError('relay-not-configured', '请在中转概览设置中填写中转 URL 和 API Key', 503)
}
...
function readCurrentRelayConfig(settings, normalizedConfig) {
  ...
  try {
    ...
    throw new PublicBalanceError('relay-config-unavailable', '暂时无法读取 Relay 配置', 503)
  }
  return resolveRelayConfig(normalizedConfig, relaySettings)
}
...
async function queryUsagePayload({ usageURLs, apiKey, fetchImpl }) {
  ...
  try { return JSON.parse(text) } catch { throw publicInvalidData('中转站返回了无法解析的数据') }
}

async function queryUsage({ usageURLs, apiKey, displayName, fetchImpl, now }) {
  const payload = await queryUsagePayload({ usageURLs, apiKey, fetchImpl })
  return normalizeSub2ApiUsage(payload, { displayName, fetchedAt: now().toISOString() })
}

function historyUsageURLs(usageURLs, observedAt) {
  const through = shanghaiDateKey(observedAt)
  const from = dateKeyDaysBefore(through, HISTORY_DAYS - 1)
  return usageURLs.map((value) => {
    const url = new URL(value)
    url.searchParams.set('days', String(HISTORY_DAYS))
    url.searchParams.set('timezone', HISTORY_TIME_ZONE)
    url.searchParams.set('start_date', from)
    url.searchParams.set('end_date', through)
    return url.href
  })
}

async function queryHistory({ usageURLs, apiKey, fetchImpl, now }) {
  const observedAt = now()
  const payload = await queryUsagePayload({ usageURLs: historyUsageURLs(usageURLs, observedAt), apiKey, fetchImpl })
  return normalizeSub2ApiHistory(payload, { now: observedAt, fetchedAt: observedAt.toISOString() })
}
...
export function createHistoryReader({ settings, credentials, config = DEFAULT_CONFIG, fetchImpl = fetch, now = () => new Date() }) {
  requireMethod(settings, 'settings', 'get')
  requireMethod(credentials, 'credentials', 'resolve')
  const normalizedConfig = normalizePluginConfig({ ...DEFAULT_CONFIG, ...config })
  let current = null
  ...
  async function readOnce() {
    const resolvedConfig = readCurrentRelayConfig(settings, normalizedConfig)
    const apiKey = await resolveApiKey(credentials, resolvedConfig.credentialRef)
    return queryHistory({ ...resolvedConfig, apiKey, fetchImpl, now })
  }
...
export function createConnectionSaver({ settings, credentials, testConnection }) {
  ...
  if (typeof testConnection !== 'function') throw new Error('dsh-relay-overview requires a connection tester')
...
export function createHistoryHandler(readHistory) {
  return createStatusHandler(readHistory)
}
...
function relaySettingsBase(normalizedConfig) {
  return {
    baseURL: normalizedConfig.baseURL,
    credentialRef: normalizedConfig.credentialRef,
    usagePath: normalizedConfig.usagePath,
  }
}
...
export function apply(ctx, config) {
  ...
  const normalizedConfig = normalizePluginConfig({ ...DEFAULT_CONFIG, ...config })
  const base = relaySettingsBase(normalizedConfig)
  ...
  const readHistory = createHistoryReader({ settings: ctx.settings, credentials: ctx.credentials, config: normalizedConfig })
  ...
  }), 'relay-overview: status route')
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: HISTORY_ROUTE_PATH,
    handler: createHistoryHandler(readHistory),
  }), 'relay-overview: history route')
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: TEST_ROUTE_PATH,
    handler: createConnectionTestHandler(testConnection),
  }), 'relay-overview: connection test route')
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: SAVE_ROUTE_PATH,
    handler: createConnectionSaveHandler(saveConnection),
  }), 'relay-overview: connection save route')
}
