...
const ROUTE_PATH = '/relay-overview/status'
...
const REFRESH_EVENT = 'relay-overview:refresh'
...
function BalanceIndicator({ wide }) {
  ...
  const showTimingTooltip = React.useCallback((event) => {
    ...
    tag.className = 'relay-overview__timing'
  ...
  if (!wide) {
    ...
    return React.createElement('button', {
      type: 'button',
      className: `relay-overview relay-overview--rail relay-overview--${tone}${state.loading ? ' is-loading' : ''}${Number.isFinite(percent) ? ' has-percent' : ''}`,
      'aria-label': label,
    ...
    React.createElement('span', {
      className: 'relay-overview__ring',
      style: { '--relay-progress': `${Number.isFinite(percent) ? percent * 3.6 : 0}deg` },
      'aria-hidden': 'true',
    }, compact),
    stale ? React.createElement('span', { className: 'relay-overview__warning', 'aria-hidden': 'true' }, '!') : null)
  }

  return React.createElement('button', {
    type: 'button',
    className: `relay-overview relay-overview--wide relay-overview--${tone}${state.loading ? ' is-loading' : ''}`,
    'aria-label': label,
    onMouseEnter: showTimingTooltip,
    onMouseLeave: hideTimingTooltip,
    onFocus: showTimingTooltip,
  ...
  React.createElement('span', { className: 'relay-overview__summary' },
    React.createElement('span', { className: 'relay-overview__amount' }, data === null ? '—' : visualAmount),
    stale ? React.createElement('span', { className: 'relay-overview__stale', 'aria-hidden': 'true' }, '!') : null,
    Number.isFinite(percent)
      ? React.createElement('span', { className: 'relay-overview__percent' }, percentText(percent))
      : null),
  Number.isFinite(percent)
    ? React.createElement('span', { className: 'relay-overview__track', 'aria-hidden': 'true' },
      React.createElement('span', { className: 'relay-overview__fill', style: { width: `${percent}%` } }))
    : null)
}

const css = `
.relay-overview{--relay-tone:var(--dsw-alias-state-business-primary,#3b82f6);box-sizing:border-box;color:var(--dsw-alias-label-primary);font:inherit;cursor:pointer;border:1px solid transparent;background:transparent;position:relative;transition:background .16s ease,border-color .16s ease,opacity .16s ease}
.relay-overview:hover{background:var(--dsw-alias-interactive-bg-hover)}
.relay-overview:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:2px}
.relay-overview--healthy{--relay-tone:#22a06b}.relay-overview--warning{--relay-tone:#d49b16}.relay-overview--danger{--relay-tone:#df4b4b}.relay-overview--neutral{--relay-tone:var(--dsw-alias-state-business-primary,#3b82f6)}.relay-overview--unavailable{--relay-tone:var(--dsw-alias-label-caption,#8a8a8a)}
.relay-overview.is-loading{opacity:.78}
.relay-overview--wide{width:100%;min-width:100%;flex:0 0 100%;border-color:var(--dsw-alias-border-l1);border-radius:9px;padding:9px 10px 10px;text-align:left;display:flex;flex-direction:column;gap:7px;align-self:center}
:where(*):has(> [data-slot="sidebar.footer.action"] > .relay-overview--wide){flex-wrap:wrap;row-gap:4px}
:where(*):has(> [data-slot="sidebar.footer.action"] > .relay-overview--rail){flex-direction:column;align-items:center;row-gap:4px}
.relay-overview__summary{width:100%;display:flex;align-items:baseline;gap:8px;line-height:18px}.relay-overview__amount{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--dsw-alias-label-primary);font-size:13px;font-weight:600;font-variant-numeric:tabular-nums}.relay-overview__percent{margin-left:auto;flex:none;color:var(--relay-tone);font-size:12px;font-weight:600;font-variant-numeric:tabular-nums}.relay-overview__stale,.relay-overview__warning{color:#d49b16;font-weight:800}
.relay-overview__track{width:100%;height:4px;overflow:hidden;border-radius:999px;background:var(--dsw-alias-bg-base)}.relay-overview__fill{height:100%;display:block;border-radius:inherit;background:var(--relay-tone);transition:width .3s ease}.relay-overview__timing{position:fixed;z-index:1000;box-sizing:border-box;width:max-content;max-width:calc(100vw - 16px);padding:7px 10px;border:1px solid var(--dsw-alias-border-l1);border-radius:7px;background:var(--dsw-specific-sidebar-fill,var(--dsw-alias-bg-base));color:var(--dsw-alias-label-primary);font:500 13px/19px var(--dsw-font-family,system-ui,sans-serif);white-space:nowrap;box-shadow:0 4px 14px rgba(0,0,0,.2);pointer-events:none;opacity:0;transform:translateX(-2px);transition:opacity .1s ease,transform .1s ease}.relay-overview__timing[data-visible="true"]{opacity:1;transform:translateX(0)}
.relay-overview--rail{width:36px;height:36px;padding:4px;border-radius:9px;display:inline-flex;align-items:center;justify-content:center}
.relay-overview__ring{width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;color:var(--dsw-alias-label-primary);font-size:8px;font-weight:700;font-variant-numeric:tabular-nums;background:radial-gradient(circle at center,var(--dsw-specific-sidebar-fill) 58%,transparent 60%),conic-gradient(var(--relay-tone) 0deg,var(--dsw-alias-border-l1) 0)}
.relay-overview.has-percent .relay-overview__ring{background:radial-gradient(circle at center,var(--dsw-specific-sidebar-fill) 58%,transparent 60%),conic-gradient(var(--relay-tone) var(--relay-progress),var(--dsw-alias-border-l1) 0)}
.relay-overview__warning{position:absolute;right:1px;top:0;width:12px;height:12px;border-radius:50%;background:var(--dsw-specific-sidebar-fill);font-size:9px;line-height:12px;text-align:center}
...
function apply(ctx) {
  ...
  ctx.effect(() => {
    ...
    tag.dataset.plugin = 'dsh-relay-overview'
    tag.textContent = css
    document.head.appendChild(tag)
    return () => tag.remove()
  }, 'relay-overview: styles')
